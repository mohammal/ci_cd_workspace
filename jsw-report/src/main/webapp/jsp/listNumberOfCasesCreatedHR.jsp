...
<form:form commandName="FunctionAppBean" action="/jsw-report/mainController/submitNumberOfCasesCreatedHR">				
				<table
...
<table
					...
					<thead>
						<b><tr class="w3-text-white w3-blue w3-center w3-medium">
							...
							<td class="w3-center">Business/ Location/ Payroll Area</td>
...
%>
	 ...
	 <c:set var="companyName" value="${listCases.company}"/>
	  <c:set var="location" value="${listCases.location}"/>
 <%
 ...
 %>
    ...
    if(location.equals((String)pageContext.getAttribute("location"))){}
    else{
    	
    
  %>
  			<tr class=" w3-medium" style="background-color:#d9b3ff"><td class="">${listCases.location}</td>
  			<td class="w3-center">Total of all ${listCases.location} Payroll Area</td>
  			<td class="w3-center">Total of all ${listCases.location} Payroll Area</td>
  			<td class="w3-center">Total of all ${listCases.location} Payroll Area</td>
    			<td>Total of all period on Table</td>
 ...
 location=(String)pageContext.getAttribute("location");
 %>
					
							<td class="w3-hide">${listCases.company}</td>
							<td class="w3-hide">${listCases.location}</td>
								<td class="w3-center">${listCases.payrollarea}</td>
...
function getCaseStudiedTable(thisVariable){
	...
	var firstColumnValue=$(thisVariable).closest('tr').children('td:first').text();
	 ...
	 var  thirdColumnValue = $(thisVariable).closest("tr").find("td:eq(2)").text();
	  
    $.ajax({                    
    	  url: '/jsw-report/NW/getCaseCreatedCompany/'+firstColumnValue+"@"+secondColumnValue+"@"+thirdColumnValue,     
    	  type: 'GET',
    	async:false,
    	cache:false,
